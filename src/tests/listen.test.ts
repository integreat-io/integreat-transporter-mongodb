...
test('should listen and recieve new document', async (t) => {
...
test('should listen and recieve updated document', async (t) => {
  const { collection, collectionName } = t.context
  const dispatch = sinon.stub().resolves({ status: 'ok' })
  const optionsWithIncoming = {
    ...options,
...
test('should listen and recieve new document with idIsUnique', async (t) => {
  const { collection, collectionName } = t.context
  const dispatch = sinon.stub().resolves({ status: 'ok' })
